---
# edifact-ls-vibj
title: INVOIC D.99B schema data
status: completed
type: feature
priority: normal
...
Transcribe INVOIC's (Invoice) real D.99B branching diagram into
SchemaNode data and register it alongside the existing D.20A schema,
for (Type: "INVOIC", Version: "D", Release: "99B", Agency: "UN"), using
...
internal/edifact/invoic_d99b.go: 53 segment groups, max depth 3. Same
shape as D.20A's INVOIC: BGM, DTM, and UNS mandatory top-level
segments, plus a mandatory top-level group (segment group 50, leading
MOA) -- confirmed by directly reading the raw source rather than
assumed from the D.20A pattern.

internal/edifact/invoic_d99b_test.go: registered, minimal conformant
pass (BGM+DTM+UNS+MOA), missing mandatory DTM, and PAI exceeding its
cap of 1 (kept in real schema order in the wire, before UNS and
segment group 50).

testdata/invoic-d99b-violation.edi + scripts/e2e_check.lua: e2e check
