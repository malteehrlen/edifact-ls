...
updated_at: 2026-09-01T15:38:40Z
...
# Acceptance Criteria
- [ ] `.goreleaser.yml` builds `./cmd/edifact-ls` for linux/amd64 and
      linux/arm64, injecting the version (edifact-ls-sbsx) into each
      binary, producing archives + a checksums file
- [ ] `.github/workflows/release.yml` runs goreleaser on `v*` tag pushes
      using the repo's default `GITHUB_TOKEN` (no extra secrets needed)
- [ ] README documents how to cut a release (tag + push) and how to
